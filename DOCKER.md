...
Локальное окружение описано в `docker-compose.yml` и использует общий файл `../configs/env/local.env`.
...
Продакшен запускается через `docker-compose.prod.yml` и общий файл `../configs/env/prod.env`.
...
В продакшене compose поднимает только `app`. База данных внешняя и задается в `../configs/env/prod.env` через:
...
Env-файлы хранятся на уровне общего проекта в `../configs/env/`: `local.env` для локальной разработки и `prod.env` для production.
...
```bash
docker compose --env-file ../configs/env/local.env config
```
