...
$nom=0;
$r = mysql_query("SELECT telefonia.*,vidcall.name_call,users.f_name,users.l_name,users.o_name,ogrn.naim from telefonia left join vidcall on telefonia.vid_call=vidcall.id left join users on telefonia.idkto=users.users_id left join ogrn on ogrn.id=telefonia.idkli $all order by telefonia.id desc Limit 50");
while($res = mysql_fetch_assoc($r))  :
    ...
    <tr>
        ...
        <td><? if($res['callmessage']!= ""){?><audio controls><source src="/voicecatalog/<?echo $res["callmessage"];?>"></audio><?} else {?>запись разговора отсуствует<?}?></td>
