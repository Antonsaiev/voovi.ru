...
if(ereg("\\.\\.",$filename)) { redirect("error.php?err=3&sid=$sid&tid=$tid&lid=$lid"); exit; }
$filename 		= $userfolder."_attachments/".md5(base64_decode($bound))."_".$filename;
if (!file_exists($localname)) { redirect("error.php?err=3&sid=$sid&tid=$tid&lid=$lid"); exit; }

if($cache) {
	if (!file_exists($filename)) { redirect("error.php?err=3&sid=$sid&tid=$tid&lid=$lid"); exit; }
	clearstatcache();
