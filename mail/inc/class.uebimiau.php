...
class UebiMiau_core {
	...
	function build_complex_body($ctype,$body) {
		...
		for($i=0;$i<count($part);$i++) {
			...
			if($rctype == "multipart/alternative") {
				...
				$filename 		= $thisattach["filename"]; //$this->user_folder."_attachments/".md5($thisattach["boundary"])."_".$thisattach["name"];
	...
	function build_attach($header,$body,$boundary,$part,$mode="info",$down=0,$nametosave="tmp.eml") {
		...
		case "info":
			...
			$temp_array["content-type"] = strtolower(trim($ctype2));
			$temp_array["content-disposition"] = strtolower(trim($Acdisp[0]));
