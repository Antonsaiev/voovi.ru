...
if(isset($attachment)) {
	...
	if(!$UM->mail_auth()) { redirect("badlogin.php?sid=$sid&tid=$tid&lid=$lid"); exit; }
...
function openmessage(attach) { window.open('readmsg.php?folder=".urlencode($folder)."&pag=$pag&ix=$ix&sid=$sid&tid=$tid&lid=$lid&attachment='+attach,'','resizable=1,top=10,left=10,width=600,heigth=500,scrollbars=1,status=0'); }
...
if(count($anexos) > 0) {
	...
	for($i=0;$i<count($anexos);$i++) {
		...
		if($anexos[$i]["content-type"] == "message/rfc822") {
			$arAttachment[] = $i;
			$anexos[$i]["normlink"] = "javascript:openmessage('".join(",",$arAttachment)."')";
		} else
